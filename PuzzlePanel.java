...
public class PuzzlePanel extends JPanel{
    ...
    private void init(int width, int height){
        ...
        // Add logo
     /*   try{
            logo = ImageIO.read(getClass().getResourceAsStream("C:/Users/Hatim/My Pictures/netstalker2.png"));
        }
    ...
    private void paint_puzzle(Graphics g){
        ...
        for(int i = 0; i < manager.getMatrixSize(); i++){
            for(int j = 0; j < manager.getMatrixSize(); j++){
                ...
                g.drawString(""+ manager.getMatrixElement(i,j),currentX+25,currentY+50);


                if(manager.scores_locations.size() != 0) {
                    if(!hasDrawn) {
                        if (manager.scores_locations.get(manager.scores_locations.size() - 1).charAt(i * 5 + j) != '0') {
                            g.drawString("" + manager.scores_locations.get(manager.scores_locations.size() - 1).charAt(i * 5 + j), currentX + 35, currentY + 50);
                        }
                        hasDrawn = true;
                    }
                }
    ...
    private void paint_amblem(Graphics g){
        ...
        g.setColor(Color.white);
        g.setFont(new Font("TimesRoman",Font.PLAIN,25));
        g.drawString("NeT Stalker",startX +50 ,startY +50 );
        g.setFont(new Font("TimesRoman",Font.PLAIN,15));
        g.drawString("11/7/2019",startX +200 ,startY +50 );
        repaint();
    }
